...
int MapReader::visualize_map(vector<state_t> x_bar, bool storeForVideo, bool visRays, SensorModel* sensor_model, bool visMeas, vector<double> z_t)
{
    ...
    if (!x_bar.empty())
    {
        for (int i=0; i < x_bar.size(); ++i)
        {
            ...
            Point particle = Point_<int>((int)(x_t1.y), (int)x_t1.x);
            circle(image, particle, 1, Scalar(0, 0, 255), 2, 8);

            if (visRays || visMeas)
            {
                double x = x_t1.x + (LASER_OFFSET/res) * cos(x_t1.theta);
                double y = x_t1.y + (LASER_OFFSET/res) * sin(x_t1.theta);
                Point ray_start = Point_<int>((int)y, (int)x);
                for (int j = 0; j < MAX_SENSOR_THETA; j+=5)
                {
                    ...
                    Point ray_end = Point_<int>((int)y_end, (int)x_end);
